 ...
 * 对话框辅助类,需要自己调用show方法
 */
public object ProgressDialogUtil {
    ...
    fun getWaitDialog(context: Context, message: String): ProgressDialog {
        ...
        if (!TextUtils.isEmpty(message)) {
            ...
            waitDialog.setProgressStyle(R.style.CustomProgressDialog)
            //解去除遮罩
            waitDialog.getWindow()?.setDimAmount(0f);
        }
        return waitDialog
    }
     ...
     * @return
     */
    fun getLoadingDialog(context: Context, message: String): LoadProgressDialog {
        val waitDialog = LoadProgressDialog(context,message)
        return waitDialog
    }
